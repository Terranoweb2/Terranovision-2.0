...
import { ArrowLeft, Check, Star, Zap, Crown, Gift, Shield, Clock, Users, QrCode, Smartphone, Play } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { PREMIUM_IMAGES, getRandomImage } from '../constants/premiumImages';

const SubscriptionPageWithTrial: React.FC = () => {
  ...
  const handleSubscribe = (planId: string) => {
    if (planId === 'trial') {
      // Pour l'essai gratuit, activation directe
      alert('🎉 Votre essai gratuit de 2 jours a été activé ! Profitez de tous les films et chaînes premium.');
      navigate('/');
    } else {
      ...
      <div className={`relative z-10 transition-all duration-1000 ${
        ...
        <div className="flex items-center justify-between p-6">
          ...
          <div className="flex items-center gap-2 px-4 py-2 bg-green-500/20 backdrop-blur-sm border border-green-500/30 rounded-xl">
            ...
            <span className="text-green-300 font-medium">Essai gratuit 2 jours disponible !</span>
          </div>
        </div>
        ...
        <div className="text-center px-6 mb-12">
          ...
          <p className="text-xl text-white/80 mb-8 max-w-3xl mx-auto leading-relaxed">
            Commencez par un <span className="text-green-400 font-semibold">essai gratuit de 2 jours</span> avec 
            accès à <span className="text-primary-400 font-semibold">tous les films</span> et plus de 
            <span className="text-accent-400 font-semibold"> 2,674 chaînes premium</span>
          </p>
          
          {/* Garanties */}
          <div className="flex flex-wrap justify-center gap-6 mb-8">
            {[
              { icon: Play, text: 'Essai gratuit 2 jours' },
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {plans.map((plan, index) => (
              <div
                ...
                } ${plan.trial ? 'ring-2 ring-green-400/50' : ''}`}
              >
                {/* Badge */}
                {plan.badge && (
                  <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
                    <div className={`px-4 py-2 ${plan.trial ? 'bg-gradient-to-r from-green-500 to-emerald-500' : 'bg-gradient-to-r from-primary-500 to-accent-500'} text-white text-sm font-bold rounded-full shadow-lg`}>
                ...
                <div className="text-center mb-8">
                  {plan.trial ? (
                    <div className="mb-4">
                      ...
                      <div className="text-sm text-white/60">2 jours d'essai</div>
                      <div className="text-xs text-green-300 mt-2">Aucune carte requise</div>
                    </div>
                  ) : (
                    <>
                ...
                >
                  {plan.trial ? 'Commencer l\'essai gratuit' : 'S\'abonner avec Wave'}
                </button>
              </div>
            ))}
          </div>
            ...
            <div className="text-center mb-8">
              ...
              <div className="flex flex-wrap justify-center gap-4">
                ...
                <div className="flex items-center gap-2 px-4 py-2 bg-purple-500/20 border border-purple-500/30 rounded-xl">
                  ...
                  <span className="text-purple-300">Essai gratuit disponible</span>
                </div>
              </div>
            </div>
        ...
        <div className="text-center px-6 pb-16">
          <p className="text-white/60 mb-4">
            Commencez par un essai gratuit de 2 jours, puis choisissez l'abonnement qui vous convient. 
...
export default SubscriptionPageWithTrial;
